...
import { StoreModalService } from 'src/app/services/modal-loja/modal-loja.service';
...
@Component({
  ...
  templateUrl: './modal-loja.component.html',
  styleUrls: ['./modal-loja.component.css'],
})
export class StoreModalComponent implements OnInit, OnChanges {
  ...
  insertStore(): void {
    ...
    if (
      ...
      this.toastr.error(
        'Não é permitido cadastrar mais que um preço de venda para a mesma loja.'
      );
    } else {
      this.toastr.error(
